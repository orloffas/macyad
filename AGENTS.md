...
- Follow the global AI-Memory policy from `/Users/zerotool/.codex/AGENTS.md`.
- For non-trivial work, use local memory first: `ctx_memory` MCP (`ctx_status`, `ctx_search`, `ctx_retrieve`, `ctx_context`) or `ctx` CLI against `/Users/zerotool/AI-Memory/`.
