...
| `SyncService.swift` | `prepareBaseline` фильтрует загруженный baseline через `pair.allSyncExcludes` — так же, как `check` фильтрует через `allCheckExcludes`. Baseline, записанный до появления паттерна, всё ещё содержит эти пути, а снимки уже нет: без фильтра планировщик читает это как локальное удаление и блокирует прогон из-за файла, который никто не синхронизирует. Центральный оркестратор sync-операций: `push`, `pull`, `check`, `applyResolutions`; baseline-aware логика блокировки; взаимодействует с `RcloneProcessRunning`, `PairSnapshotProviding`, `PairConflictStateStoring`, `LocalFolderInspecting`, `LocalConflictFileManaging`. В Live monitor проставляет timestamped маркеры через `RcloneOutputObserver`: запуск rclone, exit-код, а также post-rclone фаза `refreshBaseline` (чтобы пользователь видел, почему footer остаётся `Running…` после строки `rclone exited with code 0`) |
