...
| `ConfigurationExport.swift` | Переносимый снимок конфигурации (`schemaVersion`, `exportedAt`, `preferences`, `accounts`, `pairs`) плюс `ConfigurationImportIssue` и `ConfigurationImportPlan`. Формат намеренно переиспользует доменные модели: отдельные DTO дублировали бы их поле в поле, а совместимость обеспечивает `schemaVersion` |
...
| `SyncPair.swift` | `rcloneArtifactExcludes` (`*.partial`, `*.rclone_chunk.*`) добавляется в `allSyncExcludes`/`allCheckExcludes` всегда, в обход редактируемых пользователем списков: оборванный трансфер оставляет `<имя>.<hex>.partial`, который по определению есть только локально — он превращается в «локальное изменение» и блокирует и pull, и плановый push, пока его не удалят вручную. Центральная сущность: пара local-folder ↔ Yandex-remote с политиками, расписанием, exclusion-списками и последними датами операций; `Codable` с ручным `init(from:)`/`encode(to:)` ради миграции legacy-ключей (`isAutoPushEnabled` → `autoSyncMode`, `lastScheduledPushAttemptAt` → `lastScheduledSyncAttemptAt`); содержит `DeletePolicy` и хелперы разбора `remotePath` |
