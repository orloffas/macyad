...
- Не применяй `.fixedSize(horizontal: false, vertical: true)` к длинному тексту в этой панели. Она живёт в `detail`-колонке `NavigationSplitView`, и запрос идеальной высоты при неограниченной ширине оставляет всё окно нераскладываемым: **окно рисуется пустым целиком, вместе с сайдбаром**, хотя все элементы остаются в дереве доступности (2026-08-11). Перенос строк обеспечивать через `.frame(maxWidth: .infinity, alignment: .leading)`.
